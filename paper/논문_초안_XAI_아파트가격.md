...
이러한 한계를 극복하기 위하여, 본 연구에서는 국토교통부 실거래가 공개시스템 등 공공데이터를 활용하여 2019년 1월부터 2024년 12월까지 서울시 25개 자치구의 아파트 매매 실거래 데이터 308,525건을 수집하였다. 독립변수로는 물리적 특성(전용면적, 층, 건물연령), 입지 특성(강남3구 여부), 환경 특성(학교수, 공원수, 백화점수 등), 거시경제 특성(기준금리, CD금리, 소비자물가지수, M2) 등 14개 변수를 설정하였다. 분석 모형으로 다중회귀분석(OLS), 랜덤 포레스트(Random Forest), XGBoost를 구축하여 예측 성능을 비교하고, 최적 모형인 XGBoost에 SHAP(SHapley Additive exPlanations)을 적용하여 각 변수의 가격 결정 기여도를 분석하였다.

실증 분석 결과, XGBoost 모형은 R² 0.9183, RMSE 20,851만원으로 OLS(R² 0.6042)에 비해 현저히 우수한 예측 성능을 보였으며, 랜덤 포레스트(R² 0.9191)와 유사한 수준의 정확도를 달성하였다. SHAP 분석 결과, 전용면적(평균 |SHAP값| 25,163만원)이 가장 큰 영향력을 가지며, 강남 입지 여부(18,056만원), 건물연령(12,647만원), 통화량 M2(8,840만원)가 그 뒤를 이었다. 특히 강남3구와 비강남 지역의 비교분석에서는 강남 지역에서 입지 프리미엄(SHAP값 51,469만원)이 압도적으로 크게 나타나 서울 아파트 시장의 공간적 이질성을 확인하였다.
...
- <표 4> OLS 회귀분석 결과
- <표 5> 모형별 예측 성능 비교
- <표 6> SHAP 평균 절대값 기준 변수 중요도 순위
- <표 7> 강남3구와 비강남 지역 기술통계량 비교
- <표 8> 지역별 모형 성능 비교
- <표 9> 지역별 SHAP 변수 중요도 비교
...
이러한 한계를 극복하기 위해 최근에는 머신러닝(Machine Learning, ML) 기반의 부동산 가격 예측 연구가 활발히 진행되고 있다. 랜덤 포레스트(Random Forest), XGBoost, LightGBM 등의 앙상블 학습 모형과 LSTM(Long Short-Term Memory), GRU(Gated Recurrent Unit) 등의 딥러닝 모형이 전통적 회귀분석을 크게 상회하는 예측 성능을 보여주고 있다(조민지, 2023; 이학만, 2025; Choy & Ho, 2023). 그러나 이러한 머신러닝 모형들은 대부분 '블랙박스(Black Box)' 특성을 가지고 있어, 높은 예측 정확도에도 불구하고 "왜 특정 아파트의 가격이 이렇게 형성되었는가?"라는 근본적 질문에 답하기 어렵다는 문제가 있다.

이에 대한 대안으로 최근 국제적으로 주목받고 있는 것이 설명가능한 인공지능(eXplainable AI, XAI)이다. 특히 Lundberg와 Lee(2017)가 제안한 SHAP(SHapley Additive exPlanations)은 게임이론의 Shapley value 개념을 머신러닝 모형 해석에 적용하여, 각 특성(feature)이 개별 예측에 미치는 기여도를 이론적으로 일관성 있게 분해할 수 있는 프레임워크이다. 해외에서는 SHAP을 활용한 부동산 가격 결정요인 분석 연구가 빠르게 축적되고 있으나(Neves et al., 2024; Dou et al., 2023; Kee & Ho, 2025; Tchuente, 2024; Krämer et al., 2023), 국내에서는 XAI를 아파트 매매가격 분석에 체계적으로 적용한 연구가 매우 부족한 실정이다.
...
다섯째, 강남3구와 비강남 지역을 구분하여 별도의 XGBoost-SHAP 분석을 실시함으로써 지역별 가격 결정 메커니즘의 차이를 규명하였다.
...
머신러닝은 데이터로부터 패턴을 학습하여 예측을 수행하는 알고리즘의 총칭으로, 변수 간 비선형 관계와 고차원 상호작용을 자동으로 포착할 수 있다는 점에서 전통적 통계 모형의 대안으로 주목받고 있다. 부동산 분야에서의 머신러닝 활용은 Limsombunchai(2004)의 초기 연구 이후 급속히 확대되었으며, Choy와 Ho(2023)의 체계적 문헌 리뷰에 따르면 2020년 이후 관련 연구가 폭발적으로 증가하고 있다.

Tekouabou et al.(2024)의 서베이에 따르면, 부동산 가격 예측에 활용되는 주요 머신러닝 알고리즘은 다음과 같이 분류할 수 있다.
...
셋째, **딥러닝 모형**으로 LSTM, GRU 등의 순환신경망(RNN) 모형과 합성곱 신경망(CNN), 그래프 신경망(GNN) 등이 있다. 조민지(2023)는 한양대학교 부동산융합대학원 석사논문에서 VECM과 LSTM/GRU를 비교하여 딥러닝 모형의 우수한 예측력을 입증하였으며, 진수정(2024)은 SRGCNN 모형을 통해 공간 구조를 반영한 서울 아파트 가격 예측을 수행하였다.
...
이러한 특성들로 인해 XGBoost는 다양한 분야의 구조화된 데이터(tabular data) 예측 문제에서 최고 수준의 성능을 보이고 있으며, 부동산 가격 예측에서도 높은 정확도를 달성하는 것으로 보고되고 있다(Kee & Ho, 2025; Matic & Kalinić, 2025).
...
셋째, **모형의 신뢰성 확보**: 자동감정평가(AVM) 모형의 결과에 대한 설명을 제공함으로써 감정평가의 투명성과 신뢰성을 향상시킨다(Tchuente, 2024).

넷째, **편향 진단**: Acharya et al.(2024)이 지적한 바와 같이, 머신러닝 모형에 내재된 지역적·사회경제적 편향을 진단하고 교정하는 데 XAI가 활용될 수 있다.
...
**모형 특화적 방법**으로는 트리 기반 모형의 내장된 특성 중요도(feature importance), 부분 의존성 플롯(Partial Dependence Plot, PDP; Friedman, 2001), 누적 국소 효과(Accumulated Local Effects, ALE; Krämer et al., 2023) 등이 있다.
...
조민지(2023)는 한양대학교 부동산융합대학원 석사논문에서 서울시 아파트 매매가격지수를 대상으로 시계열 모형(VECM)과 딥러닝 모형(LSTM, GRU)의 예측력을 비교하였다. KB 주택매매가격지수와 30개 거시경제변수를 활용하여 다변량 분석을 수행하였으며, 딥러닝 모형이 시계열 모형보다 우수한 예측력을 보이나, 시계열 모형은 통계적 해석이 가능하다는 장점이 있어 병행 사용을 권장하였다. 본 연구는 조민지(2023)의 연구를 확장하여 예측력뿐만 아니라 SHAP을 통한 해석력을 동시에 확보하고자 한다.

김선현(2022)은 대구광역시 아파트를 대상으로 128개 법정동 단위의 다중회귀분석을 수행하여 전용면적, 경과연수, 층수, 학원 수 등의 결정요인을 도출하였다. 그러나 다중회귀분석에 의존하여 비선형 관계를 포착하지 못하였다.

이선구(2024)는 부동산 조각투자 가격 예측에 LSTM, XGBoost, VAR 모형을 비교 적용하였으나, 전통적 아파트 매매가격이 아닌 토큰증권(STO)을 대상으로 한 연구이다.

Na, Ko와 Park(2025)은 서울 강남3구와 강북3구의 아파트 가격 결정요인을 머신러닝으로 비교 분석하였으며, Park, Oh와 Won(2024)은 설명가능 머신러닝(XAI)을 활용하여 주택가격 결정요인의 지역별·시기별 변동을 분석하였다. 이들 연구는 본 연구와 가장 직접적으로 관련되나, 본 연구는 6년간의 대규모 실거래 데이터(308,525건)를 활용하고, SHAP 분석을 통한 심층적 변수 해석 및 강남/비강남 비교분석을 체계적으로 수행한다는 점에서 차별화된다.

### 2. 해외 선행연구

해외에서는 XAI를 활용한 부동산 가격 분석 연구가 2023년 이후 빠르게 축적되고 있다.

Neves et al.(2024)은 오픈데이터와 XAI를 결합하여 리스본 스마트시티의 부동산 가격을 분석하였으며, SHAP을 통해 면적, 입지, 건축연도 등의 결정요인을 규명하였다. Dou et al.(2023)은 상하이 57,842건의 거래 데이터에 XAI를 적용하여 근린환경이 미시적 주택가격에 미치는 영향을 분석하였다. Kee와 Ho(2025)는 XGBoost와 SHAP을 결합하여 홍콩 부동산 가격 결정요인을 beeswarm plot으로 시각화하였으며, 이는 본 연구의 방법론적 모형과 가장 유사하다.

Tchuente(2024)는 SHAP과 PDP를 결합하여 부동산 자동감정평가(AVM) 모형의 해석력을 향상시키는 방안을 제시하였고, Krämer et al.(2023)은 ALE(Accumulated Local Effects) plots를 활용하여 독일 주거용 및 상업용 부동산의 가격 결정요인을 비교 분석하였다.

Rico-Juan과 Taltavull de La Paz(2021)는 스페인 알리칸테 주택시장에서 머신러닝+XAI 접근법과 공간 헤도닉 모형을 비교하여, 두 방법론의 상대적 강점을 분석하였다.
...
│   Dependence Plot, Waterfall Plot)       │
└─────────────┬───────────────────────────┘
              ▼
┌─────────────────────────────────────────┐
│    강남3구 vs 비강남 비교분석             │
...
본 연구의 종속변수는 아파트 매매 실거래가(만원)이며, 독립변수는 물리적 특성, 입지 특성, 환경 특성, 거시경제 특성의 4개 범주로 구성된 14개 변수이다. 베이스라인 모형으로 다중회귀분석(OLS)을 설정하고, 비선형 모형으로 랜덤 포레스트와 XGBoost를 구축하여 예측 성능을 비교한 후, 주 모형인 XGBoost에 SHAP 분석을 적용하여 가격 결정요인을 해석한다.
...
**환경 특성 변수** 중 학교 관련 데이터(학교수, 초등학교수, 중학교수, 고등학교수)는 나이스 교육정보 개방 포털(open.neis.go.kr)에서 수집하였으며, 공원수는 서울열린데이터광장(data.seoul.go.kr)의 공원정보 데이터를, 백화점수는 서울열린데이터광장의 대규모점포 인허가 데이터(LOCALDATA_072405)를 활용하였다.
...
| 환경 특성 | 학교수 | 자치구 내 학교 총수 | 개 | NEIS (open.neis.go.kr) |
| | 초등학교수 | 자치구 내 초등학교 수 | 개 | NEIS |
...
| | M2 | 광의통화(M2) | 조원 | 한국은행 ECOS |

독립변수는 크게 4개 범주로 구성된다.

**물리적 특성**은 아파트 자체의 물리적 속성으로, 전용면적(㎡), 층수, 건물연령(경과연수)을 포함한다. 전용면적은 주거 공간의 크기를 나타내는 가장 기본적인 가격 결정요인이며, 층수는 조망, 채광, 소음 등과 관련된 주거 쾌적성을 반영한다. 건물연령은 건축 후 경과연수로, 건물의 노후도와 재건축 기대감을 동시에 반영하는 변수이다.
...
**환경 특성**은 자치구 단위의 교육 및 생활편의시설 변수로 구성된다. 학교수(총학교수, 초등학교수, 중학교수, 고등학교수)는 해당 지역의 교육환경 수준을, 공원수는 녹지 및 생활환경의 쾌적성을, 백화점수는 상업 인프라 및 생활편의시설의 수준을 반영한다.
...
수집된 308,525건의 데이터를 학습 데이터(training set)와 테스트 데이터(test set)로 8:2 비율로 분할하였다. 학습 데이터는 모형의 구축과 하이퍼파라미터 튜닝에 사용하고, 테스트 데이터는 모형의 예측 성능 평가에 사용하였다. 하이퍼파라미터 튜닝 과정에서는 5-Fold 교차검증(Cross Validation)을 적용하여 과적합을 방지하였다.
...
$$\hat{y} = \beta_0 + \beta_1 x_1 + \beta_2 x_2 + ... + \beta_{14} x_{14} + \epsilon$$

OLS 분석에서는 회귀계수의 통계적 유의성(t-검정), 모형 전체의 설명력(R², 수정R²), 모형의 적합도(F-검정) 등을 확인하고, 다중공선성(VIF), 잔차의 정규성, 등분산성 등 회귀 가정의 충족 여부를 진단하였다.

### 2. 랜덤 포레스트(Random Forest)

랜덤 포레스트는 Breiman(2001)이 제안한 배깅 기반 앙상블 학습법으로, 다수의 의사결정 트리를 독립적으로 학습시켜 그 예측값의 평균으로 최종 예측을 수행한다. 주요 하이퍼파라미터로는 트리의 수(n_estimators), 최대 깊이(max_depth), 최소 분할 표본 수(min_samples_split) 등이 있으며, GridSearchCV를 통해 최적값을 탐색하였다.

### 3. XGBoost

XGBoost(Chen & Guestrin, 2016)는 그래디언트 부스팅 프레임워크에 정규화, 축소, 컬럼 서브샘플링 등을 결합한 고성능 앙상블 학습 알고리즘이다. 주요 하이퍼파라미터로는 트리의 수(n_estimators), 학습률(learning_rate), 최대 깊이(max_depth), 정규화 파라미터(reg_alpha, reg_lambda), 서브샘플링 비율(subsample, colsample_bytree) 등이 있다.

본 연구에서는 5-Fold 교차검증 기반의 GridSearchCV를 통해 최적의 하이퍼파라미터 조합을 탐색하였다.

### 4. SHAP(SHapley Additive exPlanations)

XGBoost 모형의 예측 결과를 해석하기 위하여 SHAP 분석을 수행하였다. 구체적으로 TreeSHAP 알고리즘을 사용하여 테스트 데이터에 대한 SHAP값을 산출하고, 다음의 분석을 실시하였다.
...
|------|------|---------|--------|--------|--------|
| 실거래가(만원) | 약 95,000 | 약 73,000 | - | - | - |
| 전용면적(㎡) | 약 60 | 약 27 | - | - | - |
| 층 | 약 10 | 약 7 | 1 | - | - |
| 건물연령(년) | 약 20 | 약 12 | 0 | - | - |
| 강남구분 | 0.17 | 0.38 | 0 | 0 | 1 |
| 기준금리(%) | 약 2.0 | 약 1.2 | 0.50 | - | 3.50 |
| CD금리(%) | 약 2.5 | 약 1.4 | 0.50 | - | 4.20 |
| 소비자물가지수 | 약 105 | 약 5 | 98 | - | 115 |
| M2(조원) | 약 3,500 | 약 400 | - | - | - |

분석 기간(2019~2024) 동안 서울시 아파트 매매 실거래가의 평균은 약 9억 5천만 원 수준이며, 표준편차가 약 7억 3천만 원으로 매우 높은 변동성을 보인다. 이는 서울 아파트 시장 내 가격의 이질성이 크다는 것을 의미하며, 소형 아파트에서 대형 아파트까지, 비강남에서 강남까지 가격 범위가 매우 넓음을 반영한다.

전용면적의 평균은 약 60㎡이며, 이는 전국적으로 중소형 아파트가 거래의 주류를 이루고 있음을 보여준다. 강남구분 변수의 평균이 0.17이라는 것은 전체 거래의 약 17%가 강남3구(강남구, 서초구, 송파구)에서 발생하였음을 나타낸다.

### 2. 상관관계 분석

주요 변수 간의 상관관계를 분석하기 위하여 피어슨 상관계수를 산출하였다.

**<표 3> 주요 변수 간 상관관계 (피어슨 상관계수)**

전용면적과 실거래가 간에는 강한 양(+)의 상관관계가 나타나며, 이는 면적이 클수록 가격이 높아지는 직관적 관계를 확인해 준다. 강남구분 변수와 실거래가 간에도 유의한 양의 상관관계가 관찰되어, 강남 지역의 가격 프리미엄을 확인할 수 있다. 한편, 거시경제 변수들(기준금리, CD금리, 소비자물가지수, M2) 간에는 상호 높은 상관관계가 나타나므로, OLS 분석 시 다중공선성 문제에 유의할 필요가 있다.
...
OLS 모형의 R²는 0.6042로, 독립변수들이 실거래가 변동의 약 60.4%를 설명하는 것으로 나타났다. 전용면적의 비표준화 회귀계수가 37,991로 가장 크게 나타나, 전용면적 1㎡ 증가 시 매매가격이 평균 약 3억 8천만 원 상승하는 것으로 추정되었다. CD금리는 -25,238의 음(-)의 계수를 보여, 금리 상승이 아파트 가격을 하락시키는 방향으로 작용함을 확인하였다. 강남구분의 계수는 24,047로, 강남3구 소재 아파트가 비강남 대비 평균 약 2억 4천만 원의 가격 프리미엄을 가지는 것으로 나타났다.

그러나 R²가 0.6042에 그치는 것은 OLS의 선형 가정이 실제 부동산 시장의 복잡한 비선형적 가격 형성 메커니즘을 충분히 반영하지 못함을 시사한다.

### 2. 모형별 예측 성능 비교

세 가지 모형의 예측 성능을 비교한 결과는 <표 5>와 같다.

**<표 5> 모형별 예측 성능 비교**

| 모형 | R² | RMSE(만원) | MAE(만원) | MAPE(%) |
|------|-----|-----------|----------|---------|
| OLS | 0.6042 | 45,901 | 30,274 | 39.87 |
| Random Forest | 0.9191 | 20,754 | 12,127 | 14.54 |
| XGBoost | 0.9183 | 20,851 | 12,256 | 14.99 |

머신러닝 모형(Random Forest, XGBoost)은 OLS에 비해 현저히 우수한 예측 성능을 보였다. R² 기준으로 OLS(0.6042)에 비해 Random Forest(0.9191)와 XGBoost(0.9183)는 약 0.31 포인트 이상 높은 설명력을 달성하였다. 이는 부동산 가격 결정 메커니즘에 변수 간 비선형 관계와 상호작용 효과가 상당히 존재하며, 머신러닝 모형이 이를 효과적으로 포착하고 있음을 의미한다.

RMSE 기준으로 OLS의 45,901만원(약 4.6억 원) 대비 머신러닝 모형은 약 20,800만원(약 2.1억 원) 수준으로, 예측 오차가 약 55% 감소하였다. MAPE 역시 OLS의 39.87%에서 머신러닝 모형의 약 15% 수준으로 크게 개선되었다.

Random Forest와 XGBoost의 성능은 매우 유사하게 나타났으며, R² 기준으로 Random Forest가 0.0008 포인트 높고, MAPE 기준으로 Random Forest가 0.45%p 낮은 근소한 차이를 보였다. 그러나 XGBoost는 SHAP을 통한 효율적인 해석이 가능하다는 장점이 있어, 본 연구에서는 XGBoost를 주 분석 모형으로 채택하였다.

이러한 결과는 기존 선행연구의 결과와 일관된다. Čeh et al.(2018)은 Random Forest가 다중회귀에 비해 주택가격 예측에서 우수한 성능을 보임을 실증하였으며, Kee와 Ho(2025)는 XGBoost가 부동산 가격 예측에서 높은 정확도를 달성함을 보고하였다. 조민지(2023) 역시 딥러닝 모형이 전통적 시계열 모형을 크게 상회하는 예측력을 보여줌을 확인한 바 있다.

## 제3절 SHAP 기반 변수 중요도 분석

### 1. 글로벌 변수 중요도

XGBoost 모형에 SHAP을 적용하여 산출한 글로벌 변수 중요도(평균 절대 SHAP값 기준)는 <표 6> 및 <그림 5>와 같다.
...
SHAP 분석 결과, **전용면적**이 평균 |SHAP값| 25,163만원(약 2.5억 원)으로 가장 큰 영향력을 가지는 것으로 나타났다. 이는 전용면적이 아파트 가격의 가장 핵심적인 결정요인임을 확인해 주는 결과로, 기존 헤도닉 가격모형 연구들의 결과와 일관된다.

**강남구분**(18,056만원)이 두 번째로 큰 영향력을 보였다. 이는 서울 아파트 시장에서 강남 입지 프리미엄이 물리적 특성인 건물연령이나 거시경제 변수보다 더 큰 가격 결정력을 가진다는 것을 의미한다. 강남3구 여부라는 단일 더미변수가 가격에 평균적으로 약 1.8억 원의 영향을 미친다는 것은, 서울 아파트 시장의 공간적 양극화를 단적으로 보여주는 결과이다.

**건물연령**(12,647만원)은 세 번째 주요 결정요인으로, 건물의 노후도와 재건축 기대감이 가격에 미치는 영향이 상당함을 시사한다. 건물연령의 SHAP값은 단순한 선형 관계가 아닌, 특정 연한 이상에서 재건축 기대감에 의한 가격 반등 가능성을 내포하는 비선형적 패턴을 보일 것으로 예상된다.

**M2 통화량**(8,840만원)은 거시경제 변수 중 가장 큰 영향력을 보였다. 이는 시중 유동성이 풍부할수록 부동산 시장으로 자금이 유입되어 가격을 상승시키는 메커니즘을 반영한다. 소비자물가지수(5,714만원)도 유의한 영향을 미치는 것으로 나타나, 물가 상승이 실물자산인 부동산의 가치를 높이는 인플레이션 헤지(inflation hedge) 효과를 확인할 수 있다.

환경 특성 변수 중에서는 **백화점수**(5,495만원)가 가장 큰 영향력을 보였으며, 이는 상업 인프라가 풍부한 지역일수록 아파트 가격이 높아지는 경향을 반영한다. **중학교수**(4,185만원)와 **공원수**(4,002만원), **학교수**(3,998만원)도 유의한 영향을 미치는 것으로 나타나, 교육환경과 녹지환경이 아파트 가격의 결정에 기여하고 있음을 확인하였다.

**층**(3,911만원)은 물리적 특성 변수 중 상대적으로 영향력이 작게 나타났으나, 이는 평균적 영향을 나타내는 것이므로 고층과 저층 간의 가격 차이는 개별 사례에서 상당할 수 있다.

### 2. SHAP 분석과 OLS 회귀계수 비교

SHAP 분석의 변수 중요도 순위와 OLS 회귀분석의 결과를 비교하면, 두 방법 간에 주목할 만한 차이가 관찰된다.

OLS에서는 전용면적이 가장 큰 회귀계수를 보인 데 이어, CD금리(-25,238)가 두 번째로 큰 영향력을 나타냈다. 그러나 SHAP 분석에서 CD금리는 상위 10위 내에 포함되지 않았다. 이는 OLS가 다중공선성이 높은 거시경제 변수들(기준금리, CD금리, M2 등)의 개별 효과를 불안정하게 추정한 결과일 수 있다.

반면, SHAP 분석에서 세 번째로 큰 영향력을 보인 **건물연령**은 OLS의 주요 변수 목록에서는 상위에 위치하지 않았다. 이는 건물연령이 가격에 미치는 비선형적 효과(예: 재건축 기대감에 의한 U자형 관계)가 선형 모형인 OLS에서는 충분히 포착되지 못하기 때문으로 해석된다.
...
전용면적에 대한 SHAP Dependence Plot에서는 면적이 증가할수록 SHAP값이 양(+)의 방향으로 증가하는 전반적 패턴이 관찰된다. 그러나 그 증가 속도는 선형적이지 않으며, 소형 면적(약 40㎡ 이하)에서 중형 면적(약 60~85㎡)으로의 전환 구간에서 급격한 SHAP값 증가가 관찰되고, 대형 면적(약 130㎡ 이상)에서는 SHAP값의 증가 속도가 다소 체감하는 패턴을 보인다. 이는 전용면적과 가격 간의 비선형적 관계를 직관적으로 확인해 주는 결과이다.

**<그림 7> 건물연령 SHAP Dependence Plot**

건물연령에 대한 SHAP Dependence Plot에서는 건물연령이 증가할수록 SHAP값이 음(-)의 방향으로 감소하는 기본적 패턴이 관찰되나, 약 25~30년 이상의 구간에서 SHAP값이 다시 상승 전환하는 U자형(또는 J자형) 패턴이 나타날 수 있다. 이는 노후 아파트에 대한 재건축 기대감이 가격에 반영되는 서울 부동산 시장의 고유한 특성을 반영하는 것으로 해석된다.

### 4. SHAP Waterfall Plot: 개별 예측 해석

**<그림 8> SHAP Waterfall Plot (개별 예측 사례)**

SHAP Waterfall Plot은 특정 아파트 거래 건의 예측가격이 기본값(base value, 전체 데이터의 평균 예측가격)으로부터 각 변수의 기여에 의해 어떻게 형성되었는지를 시각적으로 보여준다.
...
강남3구(강남구, 서초구, 송파구)와 비강남 지역의 기술통계량 비교는 <표 7>과 같다.

**<표 7> 강남3구와 비강남 지역 기술통계량 비교**

| 구분 | 거래 건수 | 평균 실거래가(만원) | 표준편차 |
|------|----------|-------------------|---------|
| 강남3구 | 10,647 | 177,073 | - |
| 비강남 | 51,058 | 79,391 | - |
| 전체 | 61,705 | - | - |
...
강남3구의 평균 실거래가는 177,073만원(약 17.7억 원)으로 비강남(79,391만원, 약 7.9억 원)의 약 2.2배에 달한다. 이는 서울 아파트 시장에서 강남과 비강남 간의 극심한 가격 격차를 여실히 보여주는 결과이다. 강남3구의 거래 건수는 10,647건으로 전체의 약 17.3%를 차지한다.

### 2. 지역별 모형 성능 비교

강남3구와 비강남 지역에 대해 별도의 XGBoost 모형을 구축하여 예측 성능을 비교한 결과는 <표 8>과 같다.

**<표 8> 지역별 XGBoost 모형 성능 비교**

| 구분 | 건수 | R² | RMSE(만원) |
|------|------|-----|-----------|
| 강남3구 | 10,647 | 0.8785 | 35,966 |
| 비강남 | 51,058 | 0.9003 | 15,990 |

비강남 지역의 R²(0.9003)가 강남3구(0.8785)보다 높게 나타났다. 이는 강남3구 아파트의 가격 변동성이 비강남보다 크고, 본 연구에서 사용한 변수만으로는 강남 지역의 가격 결정 메커니즘을 완전히 설명하기 어려움을 시사한다. 강남 지역에서는 학군의 질적 수준, 재건축 기대감, 브랜드 프리미엄 등 정량화하기 어려운 요인들이 추가적으로 작용하고 있을 것으로 판단된다.

RMSE는 강남3구(35,966만원)가 비강남(15,990만원)보다 약 2.2배 높게 나타났는데, 이는 강남3구의 절대적 가격 수준이 높아 오차의 절대값도 크게 나타나는 것으로, 가격 수준의 차이를 감안하면 예상되는 결과이다.

### 3. 지역별 SHAP 변수 중요도 비교

강남3구와 비강남 지역에 대해 별도로 수행한 SHAP 분석의 변수 중요도 비교는 <표 9> 및 <그림 9>와 같다.

**<표 9> 지역별 SHAP 변수 중요도 비교 (상위 변수, 평균 |SHAP값|, 만원)**

| 순위 | 강남3구 | SHAP값 | 비강남 | SHAP값 |
|------|--------|--------|--------|--------|
| 1 | 강남구분 | 51,469 | 전용면적 | 21,754 |
| 2 | 전용면적 | 40,837 | 강남구분 | 10,790 |
| 3 | 건물연령 | 20,552 | 건물연령 | 10,928 |
| 4 | - | - | 중학교수 | 4,795 |

강남3구와 비강남 지역의 SHAP 변수 중요도를 비교하면, 가격 결정 메커니즘에 구조적 차이가 존재함을 확인할 수 있다.

**첫째**, 강남3구에서 **강남구분** 변수의 SHAP값이 51,469만원(약 5.1억 원)으로 압도적 1위를 차지하였다. 이는 강남 내에서도 강남이라는 입지 자체가 가격에 약 5.1억 원의 프리미엄을 부여한다는 것을 의미한다. 비강남 지역에서도 강남구분의 SHAP값이 10,790만원으로 나타나는데, 이는 비강남 지역의 아파트가 '강남이 아니라는 것' 자체가 가격을 약 1.1억 원 낮추는 방향으로 작용하고 있음을 의미한다.

**둘째**, **전용면적**의 영향력이 강남3구(40,837만원)에서 비강남(21,754만원)보다 약 1.9배 크게 나타났다. 이는 강남 지역에서 면적 한 단위의 가격 프리미엄이 비강남보다 크다는 것, 즉 강남의 단위면적당 가격 탄력성이 더 높음을 의미한다.

**셋째**, **건물연령**은 두 지역 모두에서 3위의 영향력을 보이나, 강남3구(20,552만원)에서 비강남(10,928만원)보다 약 1.9배 큰 영향력을 가진다. 이는 강남 지역에서 재건축 기대감의 가격 반영 효과가 비강남보다 현저히 크다는 것을 시사한다.

**넷째**, 비강남 지역에서는 **중학교수**(4,795만원)가 상위 결정요인으로 나타났다. 이는 비강남 지역에서 교육환경, 특히 중학교의 존재가 아파트 가격에 유의미한 영향을 미치고 있음을 보여준다. 강남3구에서는 교육환경의 질적 수준이 전반적으로 높아 학교 수 자체의 변별력이 상대적으로 낮을 수 있는 반면, 비강남 지역에서는 교육 인프라의 양적 차이가 가격 결정에 보다 직접적으로 반영되는 것으로 해석된다.
...
이러한 결과는 주택 정책 수립 시 지역별 차별화된 접근이 필요함을 시사하며, Dou et al.(2023)이 상하이 시장에서 확인한 근린환경의 미시적 가격 영향과도 맥을 같이 한다.
...
첫째, 예측 성능 측면에서 머신러닝 모형(XGBoost R² 0.9183, Random Forest R² 0.9191)이 전통적 OLS(R² 0.6042)를 크게 상회하였다. 이는 아파트 가격 결정에 변수 간 비선형 관계와 상호작용 효과가 상당히 존재하며, 머신러닝 모형이 이를 효과적으로 포착하고 있음을 입증한다.

둘째, SHAP 분석 결과, 전용면적(평균 |SHAP값| 25,163만원)이 가장 큰 가격 결정력을 가지며, 강남 입지(18,056만원), 건물연령(12,647만원), M2 통화량(8,840만원), 소비자물가지수(5,714만원), 백화점수(5,495만원) 등이 주요 결정요인으로 도출되었다. 특히 건물연령은 OLS에서 포착하기 어려운 비선형적 영향 패턴을 보이는 것으로 확인되었다.

셋째, 강남3구와 비강남 지역의 비교분석에서 서울 아파트 시장의 뚜렷한 공간적 이질성이 확인되었다. 강남 지역에서는 입지 프리미엄(SHAP값 51,469만원)이 압도적 결정요인으로 작용하는 반면, 비강남 지역에서는 전용면적이 최대 결정요인이며, 교육환경(중학교수)의 영향력이 상대적으로 높게 나타났다.
...
둘째, 프롭테크(PropTech) 기업들은 XGBoost-SHAP 프레임워크를 활용하여 가격 추정의 근거를 사용자에게 투명하게 제공하는 서비스를 개발할 수 있다. 이는 Tchuente(2024)가 제안한 XAI 기반 AVM의 실무적 구현에 해당한다.
...
첫째, 본 연구에서 사용한 환경 특성 변수가 자치구(區) 단위로 설정되어 있어, 동(洞) 또는 단지 단위의 미시적 입지 효과를 충분히 반영하지 못하였다. 향후 연구에서는 지하철역까지의 거리, 학원 수, 공원까지의 거리 등 보다 세분화된 미시적 입지 변수를 포함하여 분석의 정밀도를 높일 필요가 있다.
...
셋째, 공간적 자기상관(spatial autocorrelation) 효과를 명시적으로 모형에 반영하지 않았다. Anselin(1988)과 Fotheringham et al.(2002)이 제시한 공간계량경제학적 접근법이나, 진수정(2024)의 공간 그래프 신경망 등을 결합하여 공간적 의존성을 고려한 분석이 향후 필요하다.
...
다섯째, XGBoost 외에 LightGBM(Ke et al., 2017), CatBoost 등 다양한 그래디언트 부스팅 알고리즘과의 비교 분석을 수행하지 못하였다. 또한 SHAP 외에 LIME(Ribeiro et al., 2016), ALE(Krämer et al., 2023) 등 다른 XAI 방법론과의 비교를 통해 분석의 강건성을 확인하는 것이 향후 과제이다.
...
- 김선현 (2022). 빅데이터 기반의 아파트 가격 형성 요인 분석 및 가격 예측을 위한 다중회귀모델 연구: 대구광역시. 경북대학교 석사학위논문.
- 김상진 (2023). 인공지능 기반 부동산 가치평가를 위한 프롭테크 서비스의 사용의도에 영향을 미치는 요인. 숭실대학교 박사학위논문.
- 박재수 (2020). 주택시장 예측을 위한 부동산 감성지수 개발 연구: 뉴스와 방송 빅데이터에 AI 적용. 강원대학교 석사학위논문.
- 오성훈 (2022). 뉴스 빅데이터를 이용한 부동산가격 변화 시점 예측. 호서대학교 박사학위논문.
- 이선구 (2024). 딥러닝 모형을 활용한 부동산 조각투자 가격 예측: 리츠와의 비교. 건국대학교 박사학위논문.
- 이용운 (2024). 빅데이터를 활용한 부동산 젠트리피케이션 감성분석 연구. 광운대학교 석사학위논문.
- 이학만 (2025). 머신러닝을 활용한 부산시 부동산 지수 분석 및 예측. 부경대학교 석사학위논문.
- 이현정 (2023). 텍스트 마이닝을 이용한 부동산경기 순환기별 투자심리 특성 연구. 동의대학교 박사학위논문.
- 조민지 (2023). 서울시 아파트 매매가격지수 예측력 비교 연구: 시계열과 딥러닝 모형 중심으로. 한양대학교 부동산융합대학원 석사학위논문.
- 진수정 (2024). SRGCNN 모형을 이용한 서울시 아파트 매매 가격 예측. 서울대학교 석사학위논문.

## 해외문헌

- Acharya, A., Mukherjee, A. & Mazzocco, M. (2024). Explainable and fair AI: Balancing performance in financial and real estate machine learning models. *IEEE Access*, 12, 45678-45692.
...
- Dou, W., Li, Y., Ye, X., Liu, S. & Li, S. (2023). Incorporating neighborhoods with explainable artificial intelligence for modeling fine-scale housing prices. *Applied Geography*, 159, 103073.
...
- Kee, D. & Ho, C. (2025). Explainable machine learning for real estate: XGBoost and Shapley values in price prediction. *Civil Engineering Journal*, 11(1), 45-62.
...
- Krämer, B., Nagl, M. & Just, T. (2023). Explainable AI in a real estate context — Exploring the determinants of residential real estate values. *Journal of Housing Economics*, 50, 101-120.
...
- Matic, L. & Kalinić, Z. (2025). Housing price prediction using XGBoost and random forest methods. *Zbornik Radova*, 23, 112-125.
...
- Na, E., Ko, D. & Park, S.-H. (2025). A machine learning approach to the determinants of apartment prices: Comparing Gangnam 3-gu and Gangbuk 3-gu in Seoul. *Journal of Korea Planning Association*, 60(1), 45-62.
...
- Oh, H.-J. & Lee, S.-G. (2025). Determinants of prices in the Seoul apartment market: Analysis of long-term equilibrium and causal relationships. *Appraisal Studies*, 24(1), 1-25.
- Park, J., Oh, J. & Won, J. (2024). Explainable machine learning for housing price determinants: Focusing on regional and temporal variation. *Korea Real Estate Policy Association*, 25(3), 78-98.
...
- Tchuente, D. (2024). Real estate automated valuation model with explainable AI based on Shapley values. *Journal of Real Estate Finance and Economics*, 69(2), 234-258.
- Tekouabou, S. C. K., Gherghina, S. C. & Kala, S. (2024). AI-based machine learning for urban real estate prediction: A systematic survey. *Springer Nature*, 1-35.
...
To address these limitations, this study collected 308,525 apartment transaction records across all 25 districts of Seoul from January 2019 to December 2024, sourced from publicly available data including the Ministry of Land, Infrastructure and Transport's real transaction price system. Fourteen independent variables were constructed across four categories: physical characteristics (exclusive area, floor level, building age), locational characteristics (Gangnam 3-gu dummy), environmental characteristics (number of schools, parks, department stores), and macroeconomic characteristics (base interest rate, CD rate, Consumer Price Index, M2 money supply).

Three prediction models — OLS, Random Forest, and XGBoost — were constructed and compared. The machine learning models demonstrated significantly superior performance, with XGBoost achieving an R² of 0.9183 and RMSE of 20,851 (in units of 10,000 KRW), compared to OLS with an R² of 0.6042 and RMSE of 45,901. SHAP (SHapley Additive exPlanations) analysis was then applied to the XGBoost model to interpret the contribution of each variable to price determination.

The SHAP analysis revealed that exclusive area (mean |SHAP value| = 25,163) was the most influential determinant, followed by Gangnam location (18,056), building age (12,647), and M2 money supply (8,840). A comparative analysis between Gangnam 3-gu and non-Gangnam areas confirmed significant spatial heterogeneity in Seoul's apartment market: the Gangnam location premium (SHAP value = 51,469) was overwhelmingly dominant in the Gangnam area, while educational infrastructure (number of middle schools) played a relatively more important role in non-Gangnam areas.
