...
8 슬라이드: 표지 / Why / 방법 / Ablation / 권역×연도 SHAP /
            연도별 변화 / 한계·후속 / 마무리.
"""
import os
...
이로부터 공간 정합·시간 정합·시공간 이질성·해석 프레임워크라는 네 가지 연구 질문을 도출했습니다.''')
    ...
    add_bullets(s, 0.7, 1.7, 12.0, 5.0, [
        ...
        'Ablation 4 시나리오: A 행정동만 / B 거리·시점무관 / C 거리+시점 / D 통합',
    ], size=15)
    add_footer(s)
...
Ablation은 행정동만, 거리만 시점무관, 거리+시점, 거리+시점+행정동 통합 네 시나리오로 설계했습니다.''')

    # =========== 4. Ablation 결과 ===========
    s = prs.slides.add_slide(blank)
    add_header(s, 3, 'Ablation — 거리 전환이 시간순 R² +7.1%p 개선 견인')
    ...
    add_footer(s)
    add_notes(s, '''[Ablation 결과 · 약 50초]
Ablation 분석에서 시나리오 A는 행정동 집계만, B는 거리 변수에 시점 무관, C는 거리에 시점 정합, D는 셋을 모두 통합한 모형입니다.
