...
출력:
  figures/fig4_shap_bar.png  — Top 15 변수 중요도
  figures/fig5_shap_summary.png — 전체 SHAP Summary (beeswarm)
  figures/fig6_dep_건물연령.png — U자형 Dependence
  figures/fig7_dep_전용면적.png — 체감형 Dependence
  ...
  figures/fig11_region_shap.png — 강남 vs 비강남 Top5 비교
  figures/fig12_year_region_heatmap.png — 연도×권역 R² 히트맵
  figures/fig13_top1_timeline.png — 연도별 Top1 변화 라인
"""
import os
import numpy as np
import pandas as pd
...
LABEL_MAP = {
    ...
    # 행정동 집계 (legacy)
    '초등학교수_legacy': '초등학교수(행정동)', '중학교수_legacy': '중학교수(행정동)',
    '고등학교수_legacy': '고등학교수(행정동)', 'CCTV수_legacy': 'CCTV수(행정동)',
    '백화점수_legacy': '백화점수(행정동)', '지하철역수_legacy': '지하철역수(행정동)',
    '공원수_legacy': '공원수(행정동)', '도서관수_legacy': '도서관수(행정동)',
    '학원수_legacy': '학원수(행정동)', '어린이집수_legacy': '어린이집수(행정동)',
    # 거시
    '기준금리': '기준금리(%)', 'CD금리': 'CD금리(%)',
...
def fig_shap_bar(sv, feats, out):
    ...
    ax.set_title('<그림 4> SHAP 변수 중요도 (전체 모형 상위 15)')
...
def fig_shap_summary(sv, X, feats, out):
    ...
    plt.title('<그림 5> SHAP Summary — 변수 방향성 및 분포')
    plt.tight_layout()
...
def fig_ablation(out):
    ...
    ax.set_title('<그림 10> Ablation 시나리오별 XGB R² (분할별)')
    ax.legend(title='분할')
